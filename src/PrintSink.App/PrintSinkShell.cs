...
internal sealed class PrintSinkShell : Component
{
  ...
  private static Element SummaryGrid(int queueCount, int readyCount) =>
    (Grid(
        ...
        rows: [GridSize.Auto],
        StatCard("Queues", queueCount.ToString(System.Globalization.CultureInfo.InvariantCulture), "PDF, XPS, PS, Cloud, PWG").Grid(row: 0, column: 0),
        StatCard("Ready", readyCount.ToString(System.Globalization.CultureInfo.InvariantCulture), "Core routes").Grid(row: 0, column: 1),
        StatCard("Package", "MSIX", "WinUI 3 app").Grid(row: 0, column: 2),
        StatCard("UI", "Reactor", "Code-first shell").Grid(row: 0, column: 3)) with { ColumnSpacing = 12 })
      .Margin(0, 0, 0, 12);
  ...
  private static Element MainGrid(IReadOnlyList<VirtualEndpoint> endpoints) =>
    (Grid(
        ...
        Panel("Pipeline", FlexColumn(
          ...
          .Grid(row: 0, column: 1)) with { ColumnSpacing = 12 })
      .Margin(0, 0, 0, 12);
    ...
    Panel(
      ...
      FlexColumn(
          ...
          (FlexRow(
              ...
              Button("Open diagnostics")) with { ColumnGap = 8 }))
        with { RowGap = 12 });
    ...
    Border(
        (FlexColumn(
            ...
            TextBlock(value)
              .FontSize(26)
              .Set(text => text.FontWeight = FontWeights.SemiBold),
            Caption(caption).Foreground(SecondaryText)) with { RowGap = 6 }))
      .Padding(16)
        ...
        (FlexRow(
            (FlexColumn(
                ...
                Caption($"{endpoint.TargetFormat}: {endpoint.Description}")
                  .Foreground(SecondaryText)) with { RowGap = 4 })
              .Flex(grow: 1),
            StatusPill(endpoint.Kind))
  ...
  private static Element PipelineStep(string number, string title, string body) =>
    (FlexRow(
        ...
        (FlexColumn(
            ...
            Caption(body)
              .Foreground(SecondaryText)
              .Set(text => text.TextWrapping = TextWrapping.Wrap)) with { RowGap = 4 }))
      with { ColumnGap = 12 })
      .Margin(0, 0, 0, 12);
}
