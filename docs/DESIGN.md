...
**Target platform:** Windows 11 24H2 (build 26100) and later.
...
| 17 | Physical IPP PSA association + workflow activation | Temporary signed PSA extension INF, Microsoft IPP Class Driver queue, `windows.printSupportWorkflow` activation | Extension task + workflow task + E2E | Association probe only; physical target-stream replacement is not claimed; E2E requires signed INF publication, PSA AUMID property, IPP GetPrinterAttributes traffic, stopped/rejecting state probe, ticket validation, workflow start/pass-through diagnostics when Windows delivers PDL modification, and PrintService events |
...
| 20 | IPP attribute get behavior for installed virtual queues | `IppPrintDevice.GetPrinterAttributes` | Package command + Core adapter + E2E | E2E requires the package command to read the installed PDF virtual queue and prove `document-format-default` and `document-format-supported` are unsupported, matching v4 platform behavior |
| 21 | Multiple instances for concurrent jobs | `uap10:SupportsMultipleInstances="true"` + real simultaneous print submissions | Manifest + E2E | CI asserts overlapping diagnostics, exact PCLm and cloud queue outputs, page counts, routes, and validated artifacts |
...
| 23 | Graceful cancel / abort / fail | `PrintWorkflowSubmittedStatus`, `AbortPrintFlow(PrintWorkflowJobAbortReason.*)` | All tasks + E2E | E2E asserts corrupt-image transform failure records `Job failed` and empty output, and Job UI cancel records PDL receipt, `Job canceled`, and empty output |
| 24 | Job password option model | `JobPasswordOptions` settings model | Core + Job UI + E2E | Job UI capture is tested; virtual file output records metadata as not applicable without exposing the secret in diagnostics or documents; no physical target-stream application |
...
| 28 | PDL passthrough with IPP job-attribute compatibility | `SetPdlPassthroughWithJobAttributesSupported`, `IPdlPassthroughProvider2`, `PrintWorkflowPrinterJob3` | `PrintSupportExtensionBackgroundTask` + `PrintSupportWorkflowBackgroundTask` + `PrintSink.App` | Tracked only. PrintSink enables the capability and uses a scoped CsWinRT projection for `IppAttributeConverter` / `IPdlPassthroughProvider2`. CI submits provider-v2 PDF passthrough to the virtual PDF queue and probes the temporary physical IPP queue. When the live runtime accepts provider-v2 submission, CI requires the attribute source, encoded IPP job/operation buffer sizes, real helper job bytes, and `PrintWorkflowPrinterJob3` passthrough-with-attributes diagnostics; if printer-specific print-ticket conversion fails, PrintSink falls back to core-mapped IPP job attributes before falling back to provider v1. When Windows reports provider-v2 as unsupported, unusable, or unavailable for a physical IPP queue, CI records explicit fallback or `provider-unavailable` evidence instead of claiming row 28 support. |
